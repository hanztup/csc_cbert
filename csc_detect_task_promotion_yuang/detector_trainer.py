...
@date  : 2021/08/23 16:10
@version: 2.0
@desc  : Develop function of CSC detect with "missing/redundant" error.
"""
...
from datasets.csc_dataset import CSC_Detect_Dataset, DetectorDataset
...
class CSCDetectTask(pl.LightningModule):
    def __init__(self, args: argparse.Namespace):
        ...
        self.detect_labels = CSC_Detect_Dataset.get_labels()
    ...
    def configure_optimizers(self):
        ...
        bert_model_parameters = list(self.model.named_parameters())
        crf_model_parameters = list(self.crf.named_parameters())
        total_model_parameters = bert_model_parameters + crf_model_parameters

        optimizer_grouped_parameters = [
            {
                "params": [p for n, p in bert_model_parameters if not any(nd in n for nd in no_decay)],
                "weight_decay": self.args.weight_decay,
            },
            {
                "params": [p for n, p in crf_model_parameters if not any(nd in n for nd in no_decay)],
                "weight_decay": self.args.weight_decay,
            },
            {
                "params": [p for n, p in total_model_parameters if any(nd in n for nd in no_decay)],
                "weight_decay": 0.0,
            },
        ]
    ...
    def compute_loss(self, logits, labels, loss_mask=None):
        ...
        # crf loss
        loss = (-1) * self.crf(emissions=logits, tags=labels, mask=loss_mask)
    ...
    def _load_dataset(self, prefix="test"):
        dataset = CSC_Detect_Dataset(directory=self.args.data_dir,
                                     prefix=prefix,
...
def evaluate():
    ...
    model = CSCDetectTask.load_from_checkpoint(checkpoint_path=args.checkpoint_path,
...
def infer(sentences):
    ...
    with torch.no_grad():
        sequence_logits = model(input_ids=input_ids, pinyin_ids=pinyin_ids, )[0]

    probabilities, argmax_labels = model.postprocess_logits_to_labels(sequence_logits.view(batch_size, seq_len, -1))
    pred_sequence_labels = argmax_labels.to("cpu").numpy().tolist()
